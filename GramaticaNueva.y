...
public void actualizarTablaNegativoFloat(String lexema) {
	...
	cadena.append(lexemaNuevo);
	IAccionSemantica asFinalFloat = new ASFinalFloat();
	asFinalFloat.ejecutar(' ',cadena,lexico.tablaTokens,tablaSimbolos);
	System.out.println("referencias: " + tablaSimbolos.tablaSimbolos.get(lexemaNuevo).getCantRef());
	IF(tablaSimbolos.tablaSimbolos.get(lexema).getCantRef() == 1) {
		tablaSimbolos.tablaSimbolos.remove(lexema);
